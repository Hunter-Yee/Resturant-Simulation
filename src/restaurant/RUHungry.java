...
public class RUHungry {
	...
	// WRITE YOUR CODE HERE
        int numCat = StdIn.readInt(); //6
        categoryVar = new String[numCat];
        menuVar = new MenuNode[numCat];
        StdIn.readLine();
        for(int i =0;i<numCat;i++){   
            String catName = StdIn.readLine(); //App
            //System.out.println(catName);
            categoryVar[i]=catName;
            int numDish = StdIn.readInt(); //4
            //System.out.println("num dish"+numDish);
            StdIn.readLine();
            //MenuNode category = null;
            for(int j = 0;j<numDish;j++){
                // StdIn.readLine();
                String dishName = StdIn.readLine(); //Knight nachos
                //System.out.println("name dish"+dishName);
                int numID = StdIn.readInt(); //9
                //System.out.println("num ID"+numID);
                String allID = StdIn.readLine().trim(); //118 104 128 113 109 105 117 126 112
                ...
                Dish dish = new Dish(catName,dishName,ID);
                MenuNode category = new MenuNode(dish, menuVar[i]);     
                menuVar[i]=category;
            }
        }
    }
	...
	// WRITE YOUR CODE HERE
        ...
        Ingredient item = newNode.getIngredient();
        int ID = item.getID();
        int hashFunction = ID%stockVarSize;
        //StockNode stock = new StockNode(item, stockVar[hashFunction]);
        if(stockVar[hashFunction]==null){
            stockVar[hashFunction]=newNode;
        }
        else{
            newNode.setNextStockNode(stockVar[hashFunction]);
            stockVar[hashFunction]=newNode;
        }     
    }
	...
	// WRITE YOUR CODE HERE
        // StockNode update = new StockNode(null, null);
        StockNode update = null;
        ...
        if(update!=null){
            Ingredient ing = update.getIngredient();
            int stock = stockAmountToAdd+ing.getStockLevel();
            ing.setStockLevel(stock);
        }
    }
	...
	// WRITE YOUR CODE HERE
        //System.out.println(menuVar.length);
        for(int i = 0;i<menuVar.length;i++){
            //System.out.println("aaaa");
            MenuNode x = menuVar[i];
            while(x!=null){
                int[]stock = x.getDish().getStockID();
                double cost = 0;
                //System.out.println(cost);
                for(int j = 0;j<stock.length;j++){
                    int num = stock[j];
                    //System.out.println(num);
                    StockNode find = findStockNode(num);
                    cost+=find.getIngredient().getCost();
                }
                double price = cost*1.2;
                x.getDish().setPriceOfDish(price);
                double diff = price-cost;
                x.getDish().setProfit(diff);
                x=x.getNextMenuNode();
            }
        }
    }
	...
	// WRITE YOUR CODE HERE
        ...
        while(!StdIn.isEmpty()){
            ...
            Ingredient ing = new Ingredient(ID, ingName, amt, cost);
	...
	// WRITE YOUR CODE HERE
        ...
        // MenuNode find = new MenuNode(null, null);
        MenuNode find = findDish(dishName);
        
        if(find==null){
            return false;
        }
        Dish dish = find.getDish();
	...
	// WRITE YOUR CODE HERE
        ...
        if(checkDishAvailability(dishName, quantity)==true){
            ...
            TransactionData x = new TransactionData("order",dishName,quantity,dish.getDish().getProfit()*quantity,true);
            addTransactionNode(x);
            int[]arr = dish.getDish().getStockID();
            for(int i = 0;i<arr.length;i++){
                int remove = 0-quantity;
                updateStock(findStockNode(arr[i]).getIngredient().getName(),arr[i],remove);
            }  
        }
        else if(checkDishAvailability(dishName, quantity)==false){
            MenuNode dish = findDish(dishName);
            TransactionData x = new TransactionData("order",dishName,quantity,0,false);
            addTransactionNode(x);
            MenuNode ptr = dish;
            int in = findCategoryIndex(dish.getDish().getCategory());
            // if(ptr ==null){
            //     ptr=menuVar[i];
            // }
            // while(ptr!=null){
            ...
            //         if(ptr!=dish){
            //             TransactionData y = new TransactionData("order",ptr.getDish().getName(),quantity,ptr.getDish().getProfit()*quantity,true);
            //             addTransactionNode(y);
            //             int[]arr = ptr.getDish().getStockID();
            //             for(int j = 0;j<arr.length;j++){
            //                 int remove = 0-quantity;
            //                 updateStock(findStockNode(arr[j]).getIngredient().getName(),arr[j],remove);
            //             }
            //         }

            //     }
            //     else if(checkDishAvailability(ptr.getDish().getName(), quantity)==false){
            //         if(ptr!=dish){
            //             TransactionData y = new TransactionData("order",ptr.getDish().getName(),quantity,0,false);
            //             addTransactionNode(y);
            //         }
            //     }
            //}
            do{
                ...
                if(checkDishAvailability(ptr.getDish().getName(), quantity)==true&&ptr!=dish){
                    // if(ptr!=dish){
                        TransactionData y = new TransactionData("order",ptr.getDish().getName(),quantity,ptr.getDish().getProfit()*quantity,true);
                        addTransactionNode(y);
                        int[]arr = ptr.getDish().getStockID();
                    ...
                    else if(checkDishAvailability(ptr.getDish().getName(), quantity)==false&&ptr!=dish){
                        //if(ptr!=dish){
                            TransactionData y = new TransactionData("order",ptr.getDish().getName(),quantity,0,false);
                            addTransactionNode(y);
                        //}
	...
	// WRITE YOUR CODE HERE
        ...
        int amt = ptr.getIngredient().getStockLevel();
        if(ptr!=null&&amt>=quantity){
            available=true;
        }
        TransactionData donate = new TransactionData("donation",ingredientName,quantity,0,false);
        if(profit()>50&&available==true){
            StockNode x=findStockNode(ingredientName);
            if(x!=null){
                int ID = x.getIngredient().getID();
                int remove = 0-quantity;
                updateStock(ingredientName,ID, remove);
                donate = new TransactionData("donation",ingredientName,quantity,0,true);
            }
            else{
                donate= new TransactionData("donation",ingredientName,quantity,0,false);
            }
        }
        else{
            donate= new TransactionData("donation",ingredientName,quantity,0,false);
	...
	// WRITE YOUR CODE HERE
        ...
        StockNode x = findStockNode(ingredientName);
        if(x!=null){
            if(profit()>=x.getIngredient().getCost()*quantity){
                int ID = x.getIngredient().getID();
                updateStock(ingredientName,ID,quantity);
                double remove = 0-x.getIngredient().getCost()*quantity;
                restock = new TransactionData("restock",ingredientName,quantity,remove,true);
            }
            else{
